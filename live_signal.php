...
// Accepte action en GET (?action=...) OU en JSON body {"action": "..."}.
...
// Normalisation directions (compat avec anciennes valeurs)
...
if ($action === 'send') {
    ...
    $direction = normalize_direction((string)($body['direction'] ?? $_POST['direction'] ?? '')); // to_sender | to_viewer
        ...
        try {
            ...
            if ($rows) {
                foreach ($rows as $r) {
                    ...
                    $messages[] = [
                        'id' => (int)$r['id'],
                        // Compat: on renvoie les deux clés (msgType + msg_type)
                        'msgType' => $r['msg_type'],
                        'msg_type' => $r['msg_type'],
                        'payload' => $payload,
